...
void BCReachabilityAnalyzer::iterate_blocks() {
  ...
  GrowableArray<ciBlock *> worklist(_arena, numblocks / 4, 0, NULL);
  GrowableArray<ciBlock *> successors(_arena, 4, 0, NULL);
  ...
  while(worklist.length() > 0) {
    ...
    iterate_one_block(blk, successors);
...
void BCReachabilityAnalyzer::iterate_one_block(ciBlock *blk, GrowableArray<ciBlock *> &successors) {
  blk->set_processed();
...
void BCReachabilityAnalyzer::enqueueMethod(ciMethod *method) {
