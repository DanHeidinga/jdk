...
StaticAnalyzer::StaticAnalyzer()
    ...
    , _discovered_klasses(CURRENT_ENV->arena(), 4, 0, NULL) 
    { }
...
void StaticAnalyzer::recordReferencedType(ciType *type) {
...
void StaticAnalyzer::enqueueMethod(ciMethod *method) {
  // Commented out as the is_loaded() call asserts
  // stringStream ss;
...
void StaticAnalyzer::analyze_method(Method *initialMethod) {
    ...
    printf("\n\n_discovered_klasses: >\n");
    fflush(stdout);
    for (int i = 0; i < _discovered_klasses.length(); i++) {
      ciType *t = _discovered_klasses.at(i);
      t->print_name();
      if (t->is_klass()) {
        printf(":K:");
      }
    ...
    // TODO: loading classes needs to happen on a java thread, not a compiler thread
    // Need to transfer the unloaded ciKlasses to a java thread to force the loads
    // TODO: nagging sense that by walking the blocks in the compiler, there may be blocks
    // that aren't accessible due to deopt points related to as-yet-not-loaded classes.
    // Are we potentially missing executable paths by loading afterwards?  Do we need to pass
    // over all the methods till we hit either no unloaded klasses, or a fixed point?
    printf("\n=== Attempting to load unloaded classes\n");
    {
      Handle class_loader;
      Handle protection_domain;
      Klass *accessing_klass = initialMethod->constants()->pool_holder(); /// TODO
      class_loader = Handle(thead, accessing_klass->class_loader());
      protection_domain = Handle(thead, accessing_klass->protection_domain());

      for (int i = 0; i < _discovered_klasses.length(); i++) {
        ciType *type = _discovered_klasses.at(i);
        if (!type->is_loaded() && !type->is_primitive_type()) {
          ciKlass *k =type->as_klass();
          Klass *klazz = SystemDictionary::resolve_or_null(k->name()->get_symbol(), class_loader, protection_domain, thead->as_Java_thread());
          if (klazz == NULL) {
            k->print_name();
...
// based on BCEscapeAnalyzer
void StaticAnalyzer::iterate_one_block(ciBlock *blk, ciMethod *method, ciMethodBlocks *methodBlocks, GrowableArray<ciBlock *> &successors) {
  blk->set_processed();
